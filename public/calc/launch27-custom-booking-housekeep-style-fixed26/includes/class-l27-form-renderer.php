...
$frequency_id = intval( $_POST['frequency_id'] );
        ...
        if ( ! empty( $discount_code ) ) { $payload['discount_code'] = $discount_code; }
...
$custom_fields = array();
if (isset($form_data['custom_fields_json'])) {
  $maybe = json_decode( wp_unslash( $form_data['custom_fields_json'] ), true );
  if (is_array($maybe)) {
    $custom_fields = $maybe;
  }
}
...
$booking_payload = array(
        ...
        if ( ! empty( $form_data['discount_code'] ) ) {
            $booking_payload['discount_code'] = sanitize_text_field( $form_data['discount_code'] );
        }

        if ( ! empty( $custom_fields ) && is_array( $custom_fields ) ) {
            $booking_payload['custom_fields'] = $this->sanitize_custom_fields( $custom_fields );
        }
    ...
    public function handle_get_services() {
        ...
        $cache_key = 'l27_services_cache_v2';
        $cached = get_transient( $cache_key );
        if ( $cached ) {
            // Safety: ensure cached data is an array (services list)
            if ( ! is_array( $cached ) ) {
        ...
        foreach ( $fields as $f ) {
            ...
            if ( isset( $f['values'] ) && is_array( $f['values'] ) ) {
                ...
                foreach ( $f['values'] as $v ) {
                    ...
                    $vid = isset( $v['id'] ) ? $v['id'] : '';
                    // Keep option id as int when numeric, otherwise string
                    if ( is_numeric( $vid ) ) {
                        $vid = intval( $vid );
                    } else {
                    ...
                    if ( isset( $v['other'] ) && $v['other'] !== '' ) {
                        $item['other'] = sanitize_text_field( $v['other'] );
                    }
                    $vals[] = $item;
                }
                if ( ! empty( $vals ) ) {
    ...
    private function sanitize_extras( $extras ) {
        ...
        foreach ( $extras as $ex ) {
            ...
            // Support "other" (if any extras ever require it)
